...
ALTER TABLE Stage MODIFY noter INT DEFAULT 0;
ALTER TABLE Stage MODIFY remarques VARCHAR(200) DEFAULT 'Pas de remarques';
...
ALTER TABLE ETUDIANT ADD  annee VARCHAR(70);
ALTER TABLE ETUDIANT DROP COLUMN ANNEE;
ALTER TABLE ETUDIANT ADD  filiere VARCHAR(180);
...
SELECT *FROM STAGE;
SELECT * FROM CONVOCATION;
--get candiature associe a l etudiant
SELECT C.date_soumission,C.STATUT,C.ID_STAGE,O.TITRE,O.DUREE,O.STATUS_OFFRE,O.TYPE_STAGE,O.MODE_STAGE,E.NOM FROM CANDIDATURE C 
JOIN OFFRE_STAGE O ON O.ID_STAGE=C.ID_STAGE
JOIN ENTREPRISE  E ON E.ID_ENTREPRISE=O.ID_ENTREPRISE  --OFFRE M3A ENTREPRISE
WHERE C.ID_ETUDIANT=4 AND C.STATUT='ACCEPTE';
-----------------get data for documents 
SELECT E.USERNAME ,E.EMAIL, E.FILIERE ,E.ANNEE,O.TITRE ,O.MODE_STAGE, O.TYPE_STAGE ,O.DUREE,S.NOTER,S.REMARQUES,En.NOM  FROM STAGE S
JOIN ETUDIANT E ON S.ID_ETUDIANT=E.ID_ETUDIANT
JOIN OFFRE_STAGE O ON O.ID_STAGE=S.ID_OFFRE
JOIN ENTREPRISE En ON En.ID_ENTREPRISE=S.ID_ENTREPRISE
WHERE S.ID_STAGE=2 ;
-------get data pour la convocation
SELECT 
    E.USERNAME, 
    E.FILIERE, 
    En.NOM, 
    ...
    O.TITRE 
FROM 
    CONVOCATION C ,OFFRE_STAGE O ,ENTREPRISE En ,ETUDIANT E
JOIN 
    ETUDIANT E ON  E.ID_ETUDIANT=C.ID_ETUDIANT 
JOIN 
    ENTREPRISE En ON En.ID_ENTREPRISE = O.ID_ENTREPRISE;
--donne de convocation
SELECT E.USERNAME,E.FILIERE,En.NOM,En.ADRESSE,C.DATE_ACCEPTATION,C.ID_CONVOCATION FROM CONVOCATION C 
JOIN ETUDIANT E ON E.ID_ETUDIANT=C.ID_ETUDIANT
JOIN OFFRE_STAGE O ON O.ID_STAGE=C.ID_STAGE
JOIN ENTREPRISE En ON O.ID_ENTREPRISE=En.ID_ENTREPRISE
WHERE C.ID_CONVOCATION=28;
----donne des eleves associe a une ecole donnee
SELECT ID_ETUDIANT ,FILIERE, USERNAME, EMAIL, TELEPHONE, DATE_NES, AGE , ANNEE , A_STAGE
FROM ETUDIANT 
WHERE ID_ECOLE = 6 
GROUP BY FILIERE,ID_ETUDIANT, USERNAME, EMAIL, TELEPHONE, DATE_NES, AGE, ANNEE ,A_STAGE;

--- afficher les mdp des eleves 
UPDATE ENTREPRISE 
SET NOM='OCP' WHERE ID_ENTREPRISE =8;
COMMIT;
----------------
...
WHERE ID_ECOLE = 6;
 ...
 SELECT E.NOM AS entreprise, COUNT(E.NOM) AS nombre_etudiants
            ...
            WHERE En.ID_ECOLE = 6  AND TRIM(S.STATUS_STAGE) ='Accepté'     
            GROUP BY E.NOM;
 ----stats ecole 
 --les ecole des eleves 
SELECT E.NOM, COUNT(E.NOM) 
FROM Stage S 
JOIN ENTREPRISE E ON S.ID_ENTREPRISE = E.ID_ENTREPRISE 
JOIN ETUDIANT En ON En.ID_ETUDIANT = S.ID_ETUDIANT
WHERE En.ID_ECOLE = 6
GROUP BY E.NOM;
------   les eleves sans stage 
SELECT * FROM ETUDIANT ;
commit ;
SELECT * FROM Etudiant
WHERE  A_STAGE = 'sans stage';

 -----etudiant sans stage dans l ecole 
SELECT 
    (SELECT COUNT(*) FROM Etudiant WHERE ID_ECOLE =6) AS total_etudiants,
    (SELECT COUNT(*) FROM Etudiant WHERE TRIM(A_STAGE) = 'sans stage' AND ID_ECOLE =6) AS etudiants_sans_stage,
    (SELECT COUNT(*) FROM Etudiant WHERE TRIM(A_STAGE) <> 'sans stage' AND ID_ECOLE =6) AS etudiants_avec_stage,
    (SELECT COUNT(*) FROM Etudiant WHERE TRIM(A_STAGE) <> 'sans stage' AND ID_ECOLE =6) * 100.0 / (SELECT COUNT(*) FROM Etudiant WHERE ID_ECOLE =6) AS taux_obtention_stage
FROM dual;

SELECT * FROM ETUDIANT;
 --le nombre de stagaire 
 SELECT * FROM STAGE ;
 SELECT COUNT(*) FROM STAGE S WHERE TRIM(STATUS_STAGE) ='Accepté' AND ID_ENTREPRISE =8;
 --les ecoles des eleves qui stagent dans l entreprise 
 SELECT * FROM ECOLE;
SELECT COUNT(EL.NOM) AS nombre_etudiants, EL.NOM
FROM ETUDIANT E
JOIN STAGE S ON S.ID_ETUDIANT = E.ID_ETUDIANT
JOIN ECOLE EL ON EL.ID_ECOLE = E.ID_ECOLE
WHERE S.ID_ENTREPRISE = 8 AND S.STATUS_STAGE ='Accepté'
GROUP BY EL.NOM;
----le nom et filiere des eleves n ayant pas de stage 
SELECT USERNAME ,FILIERE FROM ETUDIANT WHERE TRIM(A_STAGE)='sans stage' AND ID_ECOLE=6;
SELECT * FROM ETUDIANT;
UPDATE ETUDIANT SET A_STAGE='a stage' WHERE ID_ETUDIANT=21;
COMMIT;
SELECT * FROM Etudiant WHERE TRIM(A_STAGE) != 'sans stage' AND ID_ECOLE =6;